...
import { createProxyUser } from "./user-proxy";
...
test("should notify followers when user is online for user proxy", () => {
  const user1 = createProxyUser("user1");
  const user2 = createProxyUser("user2");
  const user3 = createProxyUser("user3");
